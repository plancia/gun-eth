...
contract StealthAnnouncer {
    ...
    function announcePayment(
        ...
        require(msg.value >= devFee, "Insufficient dev fee");

        StealthAnnouncement memory announcement = StealthAnnouncement({
        ...
        // Invia la dev fee
        (bool sent, ) = devAddress.call{value: devFee}("");
        require(sent, "Failed to send dev fee");

        // Rimborsa l'eventuale eccesso
        if (msg.value > devFee) {
            (bool refunded, ) = msg.sender.call{value: msg.value - devFee}("");
