 ...
 * @typedef {Object} BubbleHybridProviderOptions
 ...
 */
/** @typedef {import('./BubbleBaseProvider.js').BubbleProviderOptions} BubbleProviderOptions */
/** @typedef {import('./BubbleBaseProvider.js').FileMetadata} FileMetadata */
/** @typedef {import('./BubbleGunProvider.js').FileDownloadResult} FileDownloadResult */
/** @typedef {import('./BubbleBaseProvider.js').DeleteResult} DeleteResult */

/** @typedef {Object} GunNode */
/** @typedef {GunNode & ExtendedGunNode} FileGunNode */

export class BubbleHybridProvider extends BubbleBaseProvider {
  /**
  ...
  async handleFileDownload(bubbleId, fileName, userAddress) {
    try {
      ...
      const metadata = /** @type {FileGunNode} */ (
        /** @type {unknown} */ (
    ...
    try {
      ...
      for (const fileName of files) {
        ...
        try {
          ...
          await this.putGunData(
            `bubbles/${bubbleId}/files/${fileName}/sharedWith/${targetAddress.toLowerCase()}`,
            sharedData
          );
  ...
  async handleCreateBubble(params) {
    ...
    try {
      ...
      const contractWithSigner = /** @type {any} */ (
  ...
  async handleDeleteFile(bubbleId, fileName, userAddress) {
    try {
      ...
      // Delete shared files
      const bubblePath = metadata.filePath
      ...
      const deletePromises = files
        ...
        .map((file) =>
          fs
            .unlink(path.join(bubblePath, file))
            .then(() => console.log("Shared file deleted:", file))
            .catch((err) =>
              console.error("Error deleting shared file:", file, err)
            )
        );
      ...
      if (files) {
        for (const fileName in files) {
          ...
          await new Promise((resolve) => {
            this.gun
              .get(`${gunBubblePath}/files/${fileName}/sharedWith`)
              .map()
              .once((data, key) => {
                if (key !== "_") {
                  this.gun
          ...
          await new Promise((resolve) => {
            this.gun
              .get(`${gunBubblePath}/files/${fileName}`)
              .put({ _: { "#": null } }, (ack) => {
                resolve();
              });
          });
