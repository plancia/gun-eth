...
import { ethToGunAccount, createSignature, MESSAGE_TO_SIGN } from '../src/core/gun-eth.js';
...
async function saveDecryptedFile(bubbleId, fileName, content, isBase64 = false) {
  ...
  if (isBase64) {
    ...
    try {
      ...
      console.log(`📊 Dimensione immagine decodificata: ${dataToWrite.length} bytes`);
    } catch (error) {
      console.error(`❌ Errore nella decodifica base64 per ${fileName}:`, error);
      throw error;
    }
    ...
    try {
      ...
      console.warn(`⚠️ Errore nella formattazione JSON per ${fileName}:`, error);
      dataToWrite = typeof content === 'string' ? content : JSON.stringify(content);
    }
  } else {
...
async function runClientTests() {
  ...
  try {
    ...
    console.log("JSON file verification:", isEqual ? "✅ Success" : "❌ Failed");

    if (!isEqual) {
      console.log("\nJSON Comparison:");
      console.log("Expected (normalized):", JSON.stringify(JSON.parse(normalizedExpected), null, 2));
      console.log("Got (normalized):", JSON.stringify(JSON.parse(normalizedActual), null, 2));
      console.log("\nDifferences:");
      ...
      const findDifferences = (obj1, obj2, path = '') => {
        ...
        for (const key of allKeys) {
          ...
          if (!(key in obj1)) {
            ...
            differences.push(`Type mismatch at ${currentPath}: expected ${typeof obj1[key]}, got ${typeof obj2[key]}`);
          } else if (typeof obj1[key] === 'object' && obj1[key] !== null) {
            differences.push(...findDifferences(obj1[key], obj2[key], currentPath));
          } else if (obj1[key] !== obj2[key]) {
            differences.push(`Value mismatch at ${currentPath}: expected ${obj1[key]}, got ${obj2[key]}`);
          }
        }
        return differences;
      };
      ...
      if (differences.length > 0) {
        ...
        console.log("No structural differences found - might be formatting only");
      }
    }
    ...
    console.log("PNG file verification:",
      decodedImage === imageBuffer.toString('base64') ? "✅ Success" : "❌ Failed"
    );
