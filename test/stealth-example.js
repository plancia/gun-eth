...
async function runTests() {
  ...
  await stealthExampleWithCustomContract();
  
  console.log("\n✨ All tests completed!");
}

async function stealthExampleWithCustomContract() {
  try {
    console.log("\n🎭 STEALTH PROTOCOL DEMO (Custom Contract)");
    ...
    console.log("📄 Using existing StealthAnnouncer contract...");
    const customContractAddress = '0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6';
    console.log("Contract address:", customContractAddress);

    // Inizializza StealthChain con contratto personalizzato
    const stealthChain = new StealthChain(gun, {
    ...
    // Setup Alice
    console.log("\n👩 Setting up Alice with custom contract...");
    setSigner(TEST_RPC, MOCK_KEYS.alice);
    console.log("Alice's signer configured");
    ...
    // Setup Bob
    console.log("\n👨 Setting up Bob with custom contract...");
    setSigner(TEST_RPC, MOCK_KEYS.bob);
    console.log("Bob's signer configured");
    ...
    console.log("\nGenerating stealth address for Alice using custom contract...");
    ...
    // Bob annuncia il pagamento usando il contratto personalizzato
    console.log("\n💸 Bob announcing payment using custom contract...");
    await stealthChain.announceStealthPayment(
      stealthInfo.stealthAddress,
      stealthInfo.senderPublicKey,
      stealthInfo.spendingPublicKey,
    ...
    console.log("✅ Payment announced on custom contract");

    // Alice recupera i pagamenti dal contratto personalizzato
    console.log("\n🔍 Alice checking for payments on custom contract...");
    setSigner(TEST_RPC, MOCK_KEYS.alice);
    const payments = await stealthChain.getStealthPayments(
      aliceSignature,
      { 
    ...
    if (payments.length > 0) {
      console.log("✅ Found payments on custom contract:", payments.length);
      
      // Per ogni pagamento trovato
      for (const payment of payments) {
        console.log(`\nPayment details (custom contract):`);
        console.log("Stealth Address:", payment.stealthAddress);
        console.log("Timestamp:", new Date(payment.timestamp).toLocaleString());
        console.log("Source:", payment.source);

        // Alice recupera i fondi e li trasferisce a un nuovo indirizzo
        console.log("\n💰 Alice recovering funds from custom contract...");
        ...
        try {
          ...
          console.log("✅ Funds recovered successfully from custom contract");
          ...
          await tx.wait();
          console.log("✅ Transfer completed from custom contract, tx hash:", tx.hash);
          ...
          console.error("❌ Error during fund recovery or transfer from custom contract:", error);
        }
      }
    } else {
      console.log("❌ No payments found on custom contract");
    }
